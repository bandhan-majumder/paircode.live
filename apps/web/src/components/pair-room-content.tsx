...
export function PairRoomContent({
  ...
  useEffect(() => {
    if (
      !hasConsumedOutsourcedCode.current &&
      (outSourcedCode || outSourcedLanguage)
    ) {
  ...
  const handleError = (error: { message: string }) => {
    if (error.message === "Room is full") {
      toast.error(
        "Room is already full, please contact the person sent you the link",
      );
      router.push("/");
    }
  };
  ...
  const {
    ...
    leaveRoom,
    isConnected,
    isLeaveMutationPending,
    isLeaveMutationErrror,
  } = useSocketIO({
    token,
    roomId: id,
        ...
        <div className="flex items-center gap-2 md:gap-4">
          <Tooltip>
            ...
            <TooltipContent className="w-[95vw] md:w-auto ml-2">
              <p>
                Switching languages replaces present code with default template.
              </p>
        ...
        <div className="w-full md:w-80 lg:w-96 border-b md:border-b-0 md:border-l p-3 md:p-4 flex flex-col gap-3 md:gap-4 bg-background dark:bg-[#130303] md:order-2 overflow-y-auto md:overflow-y-visible">
          {lobby ? (
            <div className="p-2 md:p-4 text-center text-sm md:text-base">
              ...
              <p className="mb-2 md:mb-4  md:mt-10">
            ...
            <div className="flex flex-row gap-2 md:gap-3 justify-center items-center mt-2 md:mt-3">
              <Tooltip>
                <TooltipTrigger
                  ...
                  aria-label={
                    localMicOff ? "Turn on microphone" : "Turn off microphone"
                  }
                >
                  {localMicOff ? (
